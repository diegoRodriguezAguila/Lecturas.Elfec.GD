...
public class LoginFieldsValidator {
	...
	{
		List<Exception> validationErrors = FieldValidator.validate("usuario", true, username, "NotNullOrEmpty");
	...
	{
		List<Exception> validationErrors = FieldValidator.validate("contraseña", false, password, "NotNullOrEmpty");
		VoidResult result = new VoidResult();
		result.addErrors(validationErrors);
		return result;
	}
