...
/**
 * Se encarga de las conexiones remotas a oracle para realizar tareas sobre el
 * dispositivo
 * 
 * @author drodriguez
 *
...
public class DeviceRDA {
	...
	/**
	 * Obtiene remotamente el estado de un dispositivo, si es que no existe en
	 * la tabla de IMEI_APP, se retorna cero como estado no disponible
	 * 
	 * @param username
	 * @param password
	 * @param IMEI
	...
	public DeviceStatus requestDeviceStatus(String username,
		...
		ResultSet rs = OracleDatabaseConnector.instance(username, password)
				.executeSelect(
						"SELECT * FROM MOVILES.IMEI_APP WHERE IMEI=" + IMEI
								+ " AND APLICACION='Lecturas Movil GD'");
		DeviceStatus result = DeviceStatus.UNABLED;
		while (rs.next()) {
			result = DeviceStatus.get(rs.getShort("ESTADO"));
		}
		rs.close();
		return result;
	}
