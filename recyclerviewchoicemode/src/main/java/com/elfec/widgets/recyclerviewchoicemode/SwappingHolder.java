 ...
 *
 * When {@link #setSelectable(boolean)} is set to true, itemView's background is
 * set to the value of selectionModeBackgroundDrawable, and its
 * StateListAnimator is set to selectionModeStateListAnimator. When it is set to
 * false, the defaultModeBackgroundDrawable and defaultModeStateListAnimator are
 * used.
 *
 * defaultModeBackgroundDrawable and defaultModeStateListAnimator default to the
 * values on itemView at the time the holder was constructed.
 *
 * selectionModeBackgroundDrawable defaults to a StateListDrawable that displays
 * your colorAccent theme color when state_activated=true, and nothing
 * otherwise. selectionModeStateListAnimator defaults to a raise animation that
 * animates selection items to a 12dp translationZ.
 *
 * (Thanks to Kurt Nelson for examples and discussion on approaches here.
 * 
 * @see <a
 *      href="https://github.com/kurtisnelson/">https://github.com/kurtisnelson/</a>
 *      )
 */
@SuppressLint("NewApi")
public class SwappingHolder extends MultiSelectorBindingHolder implements
		SelectableHolder, View.OnClickListener {
	/**
	 * Construct a new SelectableHolder hooked up to be controlled by a
	 * Multiselector.
	 *
	 * If the Multiselector is not null, the SelectableHolder can be selected by
	 * calling
	 * {@link com.bignerdranch.android.recyclerviewchoicemode.MultiSelector#setSelected(SwappingHolder, boolean)}
	 * .
	 *
	 * If the Multiselector is null, the SelectableHolder acts as a standalone
	 * ViewHolder that you can control manually by setting
	 * {@link #setSelectable(boolean)} and {@link #setActivated(boolean)}
	 *
	 * @param itemView
	 *            Item view for this ViewHolder
	 * @param multiSelector
	 *            A selector set to bind this holder to
	 */
	public SwappingHolder(View itemView, MultiSelector multiSelector) {
		...
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
			setSelectionModeStateListAnimator(getRaiseStateListAnimator(itemView
					.getContext()));
			setDefaultModeStateListAnimator(itemView.getStateListAnimator());
		}
		// Default selection mode background drawable is this
		setSelectionModeBackgroundDrawable(getAccentStateDrawable(itemView
				.getContext()));
	 ...
	 *
	 * Selectable state can be controlled manually by setting
	 * {@link #setSelectable(boolean)}.
	 *
	...
	/**
	 * Background drawable to use in selection mode. This defaults to a state
	 * list drawable that uses the colorAccent theme value when
	...
	/**
	 * Background drawable to use when not in selection mode. This defaults to
	 * the drawable that was set on {@link #itemView} at construction time.
	 * 
	 * @return A background drawable
	 */
	 ...
	 *            A background drawable
	 */
	public void setDefaultModeBackgroundDrawable(
			Drawable defaultModeBackgroundDrawable) {
	...
	/**
	 * State list animator to use when in selection mode. This defaults to an
	 * animator that raises the view when <code>state_activated==true</code>.
	 * 
	...
	/**
	 * Set the state list animator to use when in selection mode. If not run on
	 * a Lollipop device, this method is a no-op.
	 * 
	 * @param resId
	 *            A state list animator resource id. Ignored prior to Lollipop.
	 */
	public void setSelectionModeStateListAnimator(int resId) {
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
			StateListAnimator animator = AnimatorInflater
					.loadStateListAnimator(itemView.getContext(), resId);
	 ...
	 *            A state list animator
	 */
	public void setSelectionModeStateListAnimator(
			StateListAnimator selectionModeStateListAnimator) {
		mSelectionModeStateListAnimator = selectionModeStateListAnimator;
	}

	/**
	 * Get the state list animator to use when not in selection mode. This value
	 * defaults to the animator set on {@link #itemView} at construction time.
	 * 
	 * @return A state list animator
	 */
	 ...
	 *            A state list animator
	 */
	public void setDefaultModeStateListAnimator(
			StateListAnimator defaultModeStateListAnimator) {
		mDefaultModeStateListAnimator = defaultModeStateListAnimator;
	}

	/**
	 * Set the state list animator to use when in default mode. If not run on a
	 * Lollipop device, this method is a no-op.
	 * 
	 * @param resId
	 *            A state list animator resource id. Ignored prior to Lollipop.
	 */
	...
	/**
	 * Returns whether {@link #itemView} is currently in a selectable mode.
	 *
	 ...
	 * {@link #itemView}'s background drawable is swapped out for the value of
	 * {@link #getSelectionModeBackgroundDrawable()}. When not, it is set to
	 * {@link #getDefaultModeBackgroundDrawable()}. If in Lollipop or greater
	 * versions, the same applies to
	...
	public void onClick(View itemView) {
		if (mMultiSelector != null) {
			...
			mMultiSelector.notifyClickOnView(this, itemView);
		}
	}
	...
	private static Drawable getAccentStateDrawable(Context context) {
		...
		stateListDrawable.addState(
				new int[] { android.R.attr.state_activated }, colorDrawable);
	...
	private static StateListAnimator getRaiseStateListAnimator(Context context) {
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
			return AnimatorInflater
					.loadStateListAnimator(context, R.anim.raise);
		} else {
			return null;
		}
	}
