...
public class TimeZoneInfo implements Comparable<TimeZoneInfo> {
	...
	private static final Factory mSpannableFactory = Spannable.Factory
			.getInstance();
	...
	private static Formatter mFormatter = new Formatter(mSB,
			Locale.getDefault());
	...
	static private SparseArray<CharSequence> mGmtDisplayNameCache = new SparseArray<CharSequence>();
	...
	public synchronized CharSequence getGmtDisplayName(Context context) {
		...
		final long nowMinute = System.currentTimeMillis()
		...
		if (displayName == null) {
			...
			DateUtils.formatDateRange(context, mFormatter, now, now, flags,
					mTzId);
			mSB.append("  ");
			...
			if (hasFutureDST) {
				spannableText.setSpan(
						new ForegroundColorSpan(DST_SYMBOL_COLOR), symbolStart,
						symbolEnd, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
			}
	...
	private static long[] getTransitions(TimeZone tz, long time)
		...
		Field mTransitionsField = zoneInfoClass
				.getDeclaredField("mTransitions");
	...
	public int compareTo(TimeZoneInfo other) {
		if (this.getNowOffsetMillis() != other.getNowOffsetMillis()) {
			return (other.getNowOffsetMillis() < this.getNowOffsetMillis()) ? -1
					: 1;
		}
