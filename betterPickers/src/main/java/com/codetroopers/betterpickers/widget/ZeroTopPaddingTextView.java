...
public class ZeroTopPaddingTextView extends TextView {
	...
	private static final float NORMAL_FONT_PADDING_RATIO = 0.318f; // the bold
																	// fontface
																	// has less
																	// empty
																	// space on
																	// the top
	private static final float BOLD_FONT_PADDING_RATIO = 0.198f;

	private static final float NORMAL_FONT_BOTTOM_PADDING_RATIO = 0.242f;
	// the bold fontface has less empty space on the top
	private static final float BOLD_FONT_BOTTOM_PADDING_RATIO = 0.200f;

	// pre-ICS (Droid Sans) has weird empty space on the bottom
	private static final float PRE_ICS_BOTTOM_PADDING_RATIO = 0.233f;

	private static final Typeface SAN_SERIF_BOLD = Typeface.create("san-serif",
			Typeface.BOLD);
	private static final Typeface SAN_SERIF_CONDENSED_BOLD = Typeface.create(
	...
	private void init() {
		decimalSeperator = getResources().getString(
				R.string.number_picker_seperator);
		timeSeperator = getResources().getString(
				R.string.time_picker_time_seperator);
	}
	...
	public void updatePadding() {
		...
		if (getPaint().getTypeface() != null
				&& getPaint().getTypeface().equals(Typeface.DEFAULT_BOLD)) {
		...
		if (getTypeface() != null
				&& getTypeface().equals(SAN_SERIF_CONDENSED_BOLD)) {
			paddingRatio = BOLD_FONT_PADDING_RATIO;
			bottomPaddingRatio = BOLD_FONT_BOTTOM_PADDING_RATIO;
		}
		if (Build.VERSION.SDK_INT < Build.VERSION_CODES.ICE_CREAM_SANDWICH
				&& getText() != null
				&& (getText().toString().equals(decimalSeperator) || getText()
						.toString().equals(timeSeperator))) {
			bottomPaddingRatio = PRE_ICS_BOTTOM_PADDING_RATIO;
		}
		// no need to scale by display density because getTextSize() already
		// returns the font
		// height in px
