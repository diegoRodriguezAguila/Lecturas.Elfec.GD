...
public class TimePicker extends LinearLayout implements Button.OnClickListener,
	 ...
	 *            the Context required for creation
	 * @param attrs
	 *            additional attributes that define custom colors, selectors,
	 *            and backgrounds.
	 */
	public TimePicker(Context context, AttributeSet attrs) {
		...
		mNoAmPmLabel = context.getResources().getString(
	...
	private void restyleViews() {
		...
		if (mDelete != null) {
			...
			mDelete.setImageDrawable(ContextCompat.getDrawable(mContext,
					mDeleteDrawableSrcResId));
		}
	...
	protected void updateTime() {
		...
		if (mInputPointer > -1) {
			...
			if (mInputPointer > 0 && mInputPointer < 3 && hours1 != -2) {
				int digits = mInput[mInputPointer] * 10
	 ...
	 * enters "00" or "30"
	 *
	 * @return true or false whether a user is allowed to click on the left or
	 *         right
	 */
	 ...
	 * H2 : M1 M2 the value retured is H1*1000+H2*100+M1*10+M2
	 *
	 ...
	 *
	 * @param maxKey
	 *            the maximum key that can be pressed
	 */
	 ...
	 *
	 * @param b
	 *            the parent Fragment's "Set" button
	 */
