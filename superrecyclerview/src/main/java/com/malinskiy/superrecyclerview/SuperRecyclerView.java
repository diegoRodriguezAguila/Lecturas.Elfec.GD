...
public class SuperRecyclerView extends FrameLayout {
	...
	protected void initAttrs(AttributeSet attrs) {
		TypedArray a = getContext().obtainStyledAttributes(attrs,
				R.styleable.superrecyclerview);
		try {
			mSuperRecyclerViewMainLayout = a.getResourceId(
					R.styleable.superrecyclerview_mainLayoutId,
					R.layout.layout_progress_recyclerview);
			mClipToPadding = a.getBoolean(
					R.styleable.superrecyclerview_recyclerClipToPadding, false);
			mPadding = (int) a.getDimension(
					R.styleable.superrecyclerview_recyclerPadding, -1.0f);
			mPaddingTop = (int) a.getDimension(
					R.styleable.superrecyclerview_recyclerPaddingTop, 0.0f);
			mPaddingBottom = (int) a.getDimension(
					R.styleable.superrecyclerview_recyclerPaddingBottom, 0.0f);
			mPaddingLeft = (int) a.getDimension(
					R.styleable.superrecyclerview_recyclerPaddingLeft, 0.0f);
			mPaddingRight = (int) a.getDimension(
					R.styleable.superrecyclerview_recyclerPaddingRight, 0.0f);
			mScrollbarStyle = a.getInt(
					R.styleable.superrecyclerview_scrollbarStyle, -1);
			mEmptyId = a.getResourceId(
					R.styleable.superrecyclerview_layout_empty, 0);
			mMoreProgressId = a.getResourceId(
					R.styleable.superrecyclerview_layout_moreProgress,
					R.layout.layout_more_progress);
			mProgressId = a.getResourceId(
					R.styleable.superrecyclerview_layout_progress,
	...
	private void initView() {
		...
		View v = LayoutInflater.from(getContext()).inflate(
				mSuperRecyclerViewMainLayout, this);
	...
	protected void initRecyclerView(View view) {
		...
		if (recyclerView instanceof RecyclerView)
			...
			throw new IllegalArgumentException(
					"SuperRecyclerView works with a RecyclerView!");
		...
		if (mRecycler != null) {
			...
			mInternalOnScrollListener = new RecyclerView.OnScrollListener() {
				...
				public void onScrolled(RecyclerView recyclerView, int dx, int dy) {
					...
					if (((totalItemCount - lastVisibleItemPosition) <= ITEM_LEFT_TO_LOAD_MORE || (totalItemCount - lastVisibleItemPosition) == 0
						...
						if (mOnMoreListener != null) {
							...
							mOnMoreListener.onMoreAsked(mRecycler.getAdapter()
									.getItemCount(), ITEM_LEFT_TO_LOAD_MORE,
					...
					if (mExternalOnScrollListener != null)
						mExternalOnScrollListener.onScrolled(recyclerView, dx,
								dy);
					if (mSwipeDismissScrollListener != null)
						mSwipeDismissScrollListener.onScrolled(recyclerView,
								dx, dy);
				}
				...
				public void onScrollStateChanged(RecyclerView recyclerView,
					...
					if (mExternalOnScrollListener != null)
						mExternalOnScrollListener.onScrollStateChanged(
								recyclerView, newState);
					if (mSwipeDismissScrollListener != null)
						mSwipeDismissScrollListener.onScrollStateChanged(
								recyclerView, newState);
				}
			};
			...
			if (mPadding != -1.0f) {
				...
				mRecycler.setPadding(mPaddingLeft, mPaddingTop, mPaddingRight,
						mPaddingBottom);
			}
	...
	/**
	 * Set the adapter to the recycler Automatically hide the progressbar Set
	 * the refresh to false If adapter is empty, then the emptyview is shown
	 *
	 * @param adapter
	 */
	public void setAdapter(RecyclerView.Adapter<?> adapter) {
		boolean isNull = adapter == null;
		mRecycler.setAdapter(adapter);
		mProgress.setVisibility(isNull ? View.VISIBLE : View.GONE);
		mRecycler.setVisibility(isNull ? View.GONE : View.VISIBLE);
		mEmpty.setVisibility(View.GONE);
	 ...
	 * @param colRes4
	 */
	public void setRefreshingColorResources(@ColorRes int colRes1,
			@ColorRes int colRes2, @ColorRes int colRes3, @ColorRes int colRes4) {
	...
	/**
	 * Obtiene el layoutmanager del recyclerview
	 * 
